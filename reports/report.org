...
[[#sec:exp-setup]]. The authors' conducted experiments on five different datasets,
the results of which can be found in our results section. It is our aim to
...
same datasets as mentioned before is reported. In total, the authors describe 6
variants of the model in this section. As such, we will attempt to reproduce
...
to the encoder of a Variational Auto-Encoder (VAE)-like model [cite VAE] that outputs the means and
...
cite:sarhan2020fairness, the CIFAR-10 sensitive accuracy we attained is
substantially higher than theirs, as displayed in Table \ref{tab:table1}. As
for the CIFAR-100 dataset, unlike results displayed by cite:sarhan2020fairness,
our model was not able to learn a representation that carried meaningful
...
\begin{figure}
     ...
     \caption{Target and sensitive accuracies of our model in comparison
     with majority label classifier (denoted by black line) and various
     other models for Adult, German, and YaleB datasets. The bars denoted by X correspond to direct use of the
...
certain seeds, a target accuracy was obtained that was identical to the $76\%$
reported by Sarhan et al.; however, over multiple runs, we obtain a lower
accuracy around $73\%$ (see Figure \ref{fig:adult_german_yaleb}).  For YaleB,
we were not able to reproduce the accuracies reported by Sarhan et al. Instead,
our model achieved a lower target accuracy, and a sensitive accuracy more
distant from the majority label classifier, suggesting that our model's
...
\begin{figure}
     ...
     \end{subfigure}
     \caption{Target and sensitive accuracies of our model for different combinations of loss term components, results are averaged over 5 random seeds.}
     \label{fig:ablative}
\end{figure}

The results of our ablative study are shown in Figure \ref{fig:ablative}.
The baseline measurement was omitted as it was unclear from the text what it
entailed. While the target accuracies for Adult are similar to what we see in
\cite{sarhan2020fairness}, we see smaller deviations for the sensitive accuracy
across variations of the loss term. For German, we observe a lower target
accuracy that is quite consistently at $0.73$. The sensitive accuracy is in
line with what Sarhan et al. report.  The relatively large
discrepancy between our German results may be attributed to their
(reportedly) lack of random seed averaging. We have observed similar results to
what Sarhan et al. reported for German, but only for particular
random seeds: upon averaging over multiple random seeds these effects disappeared.
#+END_EXPORT
...
One of the issues we ran in is that for these models training seems very unstable,
which is evident from the high fluctuation in performance when we vary the
random seed or the  number of maximum epochs. This is not addressed in the
paper and therefore there is no information on how to deal with it. (Reflected
in high variance in results also?)
...
We did have been able to communicate with the original authors.
- Got our hands on some missing parameters, such as sizes of latent
  representations, batch sizes, step_size, and more.
- Got some clarification on how the $\lambda$ values were updated and how the
  losses are backpropagated.
- Would receive some info over their YaleB processing but did not receive this
  in the end.
