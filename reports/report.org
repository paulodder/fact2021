...
The reference work by Sarhan et al aims to work towards a method of generating
...
respect to the actual 'target' embeddings that are to be used in the downstream task.
...
section [[#sec:exp-setup]]. The authors' reported values can be found in the
results section in table [[tab:sarhan_accuracies]]. Note that some of the
accuracies were not reported exactly and have therefore been deduced from
figures. It is our aim to reproduce these accuracies found by the authors.
...
- @ Stefan: Het is nog het plan om de discriminator een andere letter te geven
  dan q to avoid confusion.
...
several datasets.
- @ Stefan: het is het plan om deze modelbeschrijvingen nog samen te vatten een
  tabel voor de sake of overzichtelijkheid.
...
the sensitive attribute. Again, we solely use accuracy as evaluation metric.

We use the following classifier networks for evaluation per dataset:

\paragraph{Tabular Datasets} For the Adult and German datasets we implement a
single logistic regression layer for the target and sensitive predictors.

\paragraph{CIFAR datasets} For the CIFAR datasets we implement a MLP with two
hidden layers for the and sensitive predictors. The hidden layers have 256 and
128 neurons.

\paragraph{YaleB dataset} For the YaleB dataset we implement a MLP with one
hidden layer with 100 neurons for the target and sensitive predictors.
...
We have listed the reported accuracies of the authors in table
[[tab:sarhan_accuracies]].
- @ Stefan: het is het idee om hier een grotere tabel van te maken als onze
  resultaten binnen zijn zodat ze makkelijk te vergelijken zijn! Zelfde voor de
  ablative studie en we denken dat we hun heatmap resultaten in de appendix
  zetten omdat het anders veel ruimte in beslag zal nemen.
