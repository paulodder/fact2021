...
# The implemented model aimed to produce embeddings of input data which are
# invariant to some sensitive attribute. This, in turn, could allow algorithms to
# make predictions which are invariant to these sensitive attributes. In order to
# construct these 'fair' embeddings, Sarhan et al. explicilty modeled the
# sensitive attributes, in order to ensure the target embeddings' invariance.

** Scope of Reproducibility
The reference work presented the results of various experiments on five
different datasets. Evaluation of the models preformance was done training two
seperate classifiers. One, dubbed the 'target predictor', predicts the target
aatribute from our models' target embeddings. The other 'sensitive predictor'
predicts the sensitive attribute from the target embeddins. We attempt to
reproduce these accuracies for all datasets.
...
paper. To this end, we constructed a code base available at
[[https://github.com/paulodder/fact2021]]. The implemented method combines two encoder networks, one maps input samples to
the target space, and the other to the sensitive space. Samples from these
...
learning, variational autoencoders and the math involved, relatively
straightforward to understand.
** What was difficult
The most difficult aspect of the project was dealing with missing
information. Many essential implementation details were missing, and the
algorithm used to train the model was unclear and incorrect, there were
inconsistencies in the pseudo code provided. Resolving these isuess all
provided significant difficulties.
...
feature.
#  This is done by explicitly learning an encoding of the sensitive
...
can be compared with the ablative study of Sarhan et al. in Appendix <<TODO>>.
The baseline measurement was omitted as it was unclear from the text what it
entailed. In comparison to Sarhan et al., for German, we see that varying loss
