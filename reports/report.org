...
of $\bm{z}_T$ and $\bm{z}_S$.

- Beginning encoder shared, splits into two specific encoders
- Both are fed through specific discriminator
- Architecture depends on the dataset.
- Trained in supervised fashion to minimize loss:
\begin{align}
...
- To ensure disantanglement we "enforce some sort of  independence among latent
...
- $\lambda_{OD}$ and $\lambda_E$ are hyperparameters that we set.
...
- For every dataset list:
  ...
  - Link to download data (do not know if this is really something we want to
    do here, we can make sure the reference includes the reference.
  - Encoder and decoder network details
** 3.4 Hyperparameters
