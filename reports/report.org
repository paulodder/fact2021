...
In order to select the best performing model in order to evaluate, two
independent selection mechanisms were implemented, but not used in the final
experiments. One, we attempted to select the best iteration of the FODVAE model
(over all epochs) by keeping track of the version in which performance was
best. We first defined performance as train target accuracy (higher is better);
then, as test target accuracy subtracted by test crossover accuracy (again,
higher is better). Two, we attemped to select the best iteration of target and
sensitive predictors during their training, again by tracking based on their
performance. Here, performance was defined as test target accuracy. However,
this augmentation was discarded as we were unsure whether this was implemented
